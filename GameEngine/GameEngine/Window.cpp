...
float hero_movement_scale = 0.05f;
...
//    std::cout << "num Asteroids passed: " << asteroidGroup->num_of_asteroids_passed << std::endl;
    ...
    dragon->flap();
    dragon->move(moveLeft, moveRight, moveUp, moveDown, hero_movement_scale);
...
void Window::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if(action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        {
            moveLeft = true;
//            dragon->translate(-hero_movement_scale,0.0f,0.0f);
        }
...
//            dragon->translate(hero_movement_scale,0.0f,0.0f);
        ...
        {
            moveUp = true;
//            dragon->translate(0.0f,0.0f,hero_movement_scale);
        }
